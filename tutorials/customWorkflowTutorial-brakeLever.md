...
This guide provides a step-by-step process for building a custom `solids4foam`
...
- A pressure of 50,000 Pa is applied to the green surface.
- The lever is made of aluminum with a Young's modulus of 70 GPa and a Poisson's
ratio of 0.33.
...
where you should replace`<PATH_TO_SOLIDS4FOAM>` with the location
 of location of the solids4foam directory on your system.
Optionally, make it permanent by adding it to `~/.bashrc`:
...
boundary patch.
We will combine them into a single file for the mesher:

```bash
cat *.stl > brakeLever.stl
```

Before meshing, it's a good practice to inspect the geometry in ParaView.
The `touch` command creates a dummy file that allows ParaView to recognize the
...
We'll use the `plateHole` tutorial:
...
Next, let's check the geometry file. The `surfaceCheck` utility provides
...
suggest the units are in millimeters. We need to scale the geometry to meters
...
Now, we can adjust `system/meshDict` for our first meshing attempt.
...
You'll notice that the sharp edges of the geometry are rounded or "decimated" in
...
degrees or more. Before re-running the mesher it is necesary to change surface
...
inspection, we should perform a formal quality check using the
`checkMesh` utility:
...
(like skewness and non-orthogonality) for visualization in ParaView.
```
...
The default trehold values can be found at
...
- **`load`**: A pressure of 50000 Pa is applied to this surface.
...
Next, we need to adjust input dicts in `constant` directory.
Material properties are defined in `constant/mechanicalProperties`.
We will specify the properties for aluminum:
...
control the solver's behavior.
We will use `linearGeometryTotalDisplacement` solver,
 which is a segregated solver for small strains and rotations.
...
The other files in the `constant` directory
(`physicalProperties`, `dynamicMeshDict`)
can remain unchanged. The `g` file can be modified if you wish to include
gravitational effects.

### Numerical Schemes and runTime Control

The discretization schemes in `system/fvSchemes` are mostly suitable, but it's
important to ensure they are compatible with your OpenFOAM version.
Different versions may use different names for gradient schemes
(e.g., `pointCellsLeastSquares`). The `solids4Foam::convertCaseFormat` command,
which we will add to the `Allrun` script later,
automatically handles these updates.
...
here, we use `0.05` as it provides a smoother convergence behavior for this
specific problem.
```
...
To run the case, we will modify existing `Allrun` script.
...
solvers due to the strong coupling between displacement components.
As a result, a higher number of outer correctors (`nCorr`) is required to reach
convergence.
